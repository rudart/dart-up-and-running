...
HttpRequest.getString(url)
  ...
  .catchError((e) {
    // Обрабтать или проигнорировать ошибку.
  });
```
...
> **Примечание**
> 
>Обязательно вызовите `catchError()` для результата `then()`, а не для результата оригинального `Future`. `catchError()` обрабатывает ошибки только из оригинального `Future`, а ошибки обработчика `then()` он не обрабатывает, поэтому для обработчика `then()` нужно также предусмотреть свой `catchError()`.
