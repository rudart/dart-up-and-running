#dart:core. Коллекции
Dart изначально содержит коллекцию основных API, которая включает классы для управления списками(lists), рядами(sets) и хешами(maps).

## Списки (Lists)
В качестве ознакомления использовался [литерал для создания списка](https://www.dartlang.org/docs/dart-up-and-running/contents/ch02.html#lists). Так же для этого можно использовать конструктор. Класс List так же определяет несколько методов для добавления и удаления элементов из списков.

```
// Использования конструктора List.
...
Сортировка в списке производится с помощью метода sort(). И есть возможность предоставить функцию сортировки, которая будет сравнивать два объекта. Эта функция должна возвращать < 0 для значений меньше нуля, 0 для схожего значения и > 0 для большего значения. В следующем примере используется функция compareTo(), которая осуществляет [сравнение](http://api.dartlang.org/dart_core/Comparable.html):
...
## Ряды Sets
В Dart ряды представляют собой неупорядоченный набор уникальных элементов. Поскольку элементы не упорядочены, нет возможности получать их по индексу (позиции).
...
### Общие для коллекции методы
Ряды, списки и хеши содержат общие функциональные возможности которые встречаются во многих коллекциях. Некоторые из этих общих функций определяются классом Iterable, который реализуют списки и ряды.


> **Примечание**
>
> Хоть хеши и не реализуют интерфейс Iterable, Вы можете получить итерацию из этого класса с помощью ключей хеша и свойств значений.

Используйте isEmpty для проверки не содержат ли ряды, списки и хеши значения.
```
var teas = ['green', 'black', 'chamomile', 'earl grey'];
assert(!teas.isEmpty);
```

Для того что бы применить функцию к каждому элементу списка, ряда или хеша можно использовать метод forEach():
```
var teas = ['green', 'black', 'chamomile', 'earl grey'];
...
Что бы посмотреть полный список методов, обратитесь к документации [Iterable API](http://api.dartlang.org/dart_core/Iterable.html), а так же рядов, списков и хешей.